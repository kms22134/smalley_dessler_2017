...
def netConfig(latRange,vertRange,var,lat,lev):
    #*******************************************************************************************
    ...
    from numpy import cos,pi,max,min,squeeze,average

    cosLat = cos(lat*(pi/180.))
    lev = lev
    var = average(var[:,:,(lat>=min(latRange))&(lat<=max(latRange))],axis = 2,weights = cosLat[(lat>=min(latRange))&(lat<=max(latRange))])
...
def main():
    ...
    for cv in model:
       ...
       and experiment == 'CCMVal-2'
            ...
            finUa = '/co1/ksmalley/CCMVal/' + cv + '/R2/CCMVal2_REF-B2_' + cv +'_' + num + '_T3M_ua.nc'##zonal wind file
            finT  = '/co1/ksmalley/CCMVal/' + cv + '/R2/CCMVal2_REF-B2_' + cv +'_' + num + '_T3M_ta.nc'##temperature file
            finW  = '/co1/ksmalley/CCMVal/' + cv + '/R2/CCMVal2_REF-B2_' + cv +'_' + num + '_T3M_H2O.nc'##water vapor file
            finL  = '/co1/ksmalley/CCMVal/' + cv + '/R2/CCMVal2_REF-B2_' + cv +'_' + num + '_T3M_tntlw.nc'##longwave heat flux
            finS  = '/co1/ksmalley/CCMVal/' + cv + '/R2/CCMVal2_REF-B2_' + cv +'_' + num + '_T3M_tntsw.nc'##shortwave heat flux
        elif(
             cv == 'GEOSCCM'
         and experiment == 'CCMVal-2'
            ):
         ...
         and cv != 'NIWA-UKCA'
            ...
            finA   = '/home/ksmalley/multiple_regression/data/' + 'CCMI_monthly_' + cv + '_refC2_allVars.nc'##all variables for ALL CCMI-1 models EXCEPT NIWA-UKCA
        elif(cv == 'NIWA-UKCA'):
            finAA  = '/home/ksmalley/multiple_regression/data/' + 'CCMI_monthly_' + cv + '_refC2_allVars_22.nc'##NIWA-UKCA
            finA   = '/home/ksmalley/multiple_regression/data/' + 'CCMI_monthly_' + cv + '_refC2_allVars.nc'
        if(
           cv != 'GEOSCCM'
       and experiment == 'CCMVal-2'
          ):
       ...
       and experiment == 'CCMVal-2'
            ...
            ua = netConfig(lR,(48,52),ua,latU,pLev)##average meridionally (along the longitude dimension)
         ...
         and experiment == 'CCMVal-2'
          ...
          or cv == 'GEOSCCM_CCMI'
            ...
            ua = aD.variables['ua'][timI,:,:,:]##zonal wind variable
            ua = ma.average(ua,axis = 3)##create a zonal average (average along the longitude dimension)
            ua = netConfig(lR,(45,54),ua,latU,hP)##average meridionally (along the longitude dimension)
            ua = ua/ua.std()##standardize zonal wind variable, for qbo calculation
            ua = ua.reshape(num_years,12)##reshape from a 1D array of the same size as the time dimension to a 2D array of size (number of years, number of months) i.e. (100,12)
            ...
            ua = aD.variables['ua'][timI,:,:,:]##zonal wind variable
            ua = ma.average(ua,axis = 3)##create a zonal average (average along the longitude dimension)
            ua = netConfig(lR,(47,52),ua,latU,hP)##average meridionally (along the longitude dimension)
            ua = ua/ua.std()##standardize zonal wind variable, for qbo calculation
            ua = ua.reshape(num_years,12)##reshape from a 1D array of the same size as the time dimension to a 2D array of size (number of years, number of months) i.e. (100,12)
            ...
            ua = aD.variables['ua'][timI,:,:,:]##zonal wind variable
            ua = ma.average(ua,axis = 3)##create a zonal average (average along the longitude dimension)
            ua = netConfig(lR,(45,52),ua,latU,pLev/100.)##average meridionally (along the longitude dimension)
            ua = ua/ua.std()##standardize zonal wind variable, for qbo calculation
            ua = ua.reshape(num_years,12)##reshape from a 1D array of the same size as the time dimension to a 2D array of size (number of years, number of months) i.e. (100,12)
        ...
        if(teraA):
            ...
            netRA = (tntlwA + tntswA)*86400.*((1000./80.)**(2./7.))##net heatflux trended annual anomolies
